...
const movieUrl = (id?: string | number) =>
  `${API_URL}movie/${id}?api_key=${API_KEY}`;
const creditsUrl = (id?: string) =>
  `${API_URL}movie/${id}/credits?api_key=${API_KEY}`;
const genreUrl = (id?: string) =>
  `${API_URL}discover/movie?api_key=${API_KEY}&language=en-US&sort_by=vote_count.desc&include_adult=false&include_video=false&with_genres=${id}`;
const peopleUrl = (id?: string) =>
  `${API_URL}person/${id}?api_key=${API_KEY}&language=en-US`;
const knownForUrl = (id?: string) =>
...
// Now Playing Movie IDs
const NOW_PLAYING_MOVIE_IDS = [
